package kolokviumski1.EvaluatorNaKandidati;
...
class EvaluatorBuilder {
    public static Evaluator build(Evaluator.TYPE type) throws InvalidEvaluation {
        if (type == Evaluator.TYPE.NO_CRIMINAL_RECORD) {
            return applicant -> applicant.hasCriminalRecord()==false;
        }
        ...
        if (type == Evaluator.TYPE.NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE) {
            return applicant -> (applicant.hasCriminalRecord() == false && applicant.getEmploymentYears() >= 10);
        }
        if (type == Evaluator.TYPE.NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE) {
            return applicant -> (applicant.hasCriminalRecord() == false && applicant.getCreditScore() >= 500);

        }
        if (type == Evaluator.TYPE.INVALID) {
            throw new InvalidEvaluation();

        } else return applicant -> (applicant.getEmploymentYears() >= 10 && applicant.getCreditScore() >= 500);
